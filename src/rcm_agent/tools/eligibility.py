...
def flag_coverage_gaps(eligibility_result: dict[str, Any]) -> list[str]:
    ...
    if eligibility_result.get("member_status") == "terminated":
        gaps.append("Coverage terminated; verify termination date.")
    if eligibility_result.get("termination_date"):
        gaps.append(f"Policy has termination date: {eligibility_result['termination_date']}.")
